...
try:
    ...
    from zayde_monitor.gi_composites import GtkTemplate

import colorsys as cs
from math import exp

from zayde_monitor import files_dir, icon_file
...
def sidePaneUpdate(self):
    ...
    for i in range(0,self.numOfDisks):
        try:
            ...
            self.diskSidepaneWidgetList[i].givedata(self,i)
            self.diskSidepaneWidgetList[i].disksidepanedrawarea.queue_draw()
        except Exception:
            continue
    ...
    if(len(self.netNameList)!=0):
        for i in range(0,self.numOfNets):
            try:
                ...
                self.netSidepaneWidgetList[i].givedata(self,i)
                self.netSidepaneWidgetList[i].netsidepanedrawarea.queue_draw()
            except Exception:
                continue
    ...
    if(self.isNvidiagpu==1):
        try:
            ...
            self.gpuSidePaneWidget.givedata(self)
            self.gpuSidePaneWidget.gpusidepanedrawarea.queue_draw()
        except Exception:
            pass
