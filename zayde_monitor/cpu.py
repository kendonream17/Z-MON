...
def cpuInit(self):
    ...
    try:
        ...
        self.cpuInfoLabel.set_text("Unknown CPU")
    ...
    try:
        ...
        self.cpuVirtualisationLabelValue.set_text("NA")

    # CPU Caches
    try:
        p=popen('lscpu|grep -i -m1 "L1d"')
        self.cpuL1LabelValue.set_text(sub(r"[\s]","",p.read().split(':')[1]))
        p.close()

        p=popen('lscpu|grep -i -m1 "L2"')
        self.cpuL2LabelValue.set_text(sub(r'[\s]','',p.read().split(':')[1]))
        p.close()

        p=popen('lscpu|grep -i "L3"')
        self.cpuL3LabelValue.set_text(sub(r'[\s]','',p.read().split(':')[1]))
        p.close()
    except Exception:
        self.cpuL1LabelValue.set_text("NA")
        self.cpuL2LabelValue.set_text("NA")
        self.cpuL3LabelValue.set_text("NA")
...
def cpuUpdate(self):
    ...
    try:
        ...
        self.cpuThreadsLabelValue.set_text(sub(r'[\s]','',p.read()))
        p.close()
    except Exception:
        self.cpuThreadsLabelValue.set_text("NA")
