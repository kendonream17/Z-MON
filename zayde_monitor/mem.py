...
def memorytabinit(self):
    ...
    try:
        ...
        p=popen('dmidecode -t memory 2>/dev/null | grep -E -i "memory speed"')
        ...
        self.memSpeedLabelValue.set_text('NA')
        self.memSlotLabelValue.set_text('NA')

    # Memory Form factor using shell
    try:
        # p=popen('echo '+self.passs+'| sudo -S dmidecode -t memory|grep -E -m1 -i "form factor"')
        p=popen('dmidecode -t memory 2>/dev/null | grep -E -m1 -i "form factor"')
        self.memFormLabelValue.set_text(re.sub(r'\s','',p.read().split(':')[1]))
        p.close()
    except Exception:
        self.memFormLabelValue.set_text('NA')
    ...
    try:
        ...
        p.close()
        self.memCourruptedLabelValue.set_text(re.sub(r'\s','',tempcourrupted))
    except Exception:
        self.memCourruptedLabelValue.set_text('NA')
