...
def fun(filename):
    try:
        ...
        not_found=g.Dialog(title="Can't Plot", modal=True)
    ...
    try:
        with open(filename) as ifile:
            headers=[header.strip() for header in ifile.readline().strip().split(",")]
            data=[[] for _header in headers]
        ...
        fig, axs = plt.subplots(len(data), 1, squeeze=False)
        ...
        for index, series in enumerate(data):
            axis=axs[index][0]
            header=headers[index] if index < len(headers) else f"Metric {index + 1}"
            axis.set_ylabel(header)
            axis.set_xlabel('Sample Number')
            axis.plot(n, series)
            if len(series) > 4:
                avg=[sum(series[i-2:i+3])/step for i in range(2,len(series)-2)]
                axis.plot(navg, avg)
                axis.legend(['Normal',"Average"])
        plt.show()
    except Exception as e:
        print(f"error can't plot: {e}")
        can_not_plot=g.Dialog(title="Can't Plot", modal=True)
...
#     except ImportError:
#         print("matplotlib not found")
#         not_found=g.Dialog(title="Can't Plot",parent=None)
#         content=not_found.get_content_area()
#         label=g.Label()
#         label.set_markup(
#             """
...
#         plot_dialog=g.Window(title="Z-MON Log Plot")
...
#     except Exception as e:
#         print(f"error can't plot: {e}")
#         can_not_plot=g.Dialog(title="Can't Plot",parent=None)
#         content=can_not_plot.get_content_area()
#         label=g.Label()
#         label.set_markup(
#             """
...
# plot_log("/home/neeraj/z_mon_log/code.csv")
import sys
fun(sys.argv[1])
