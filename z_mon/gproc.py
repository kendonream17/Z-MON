...
try:
    ...
    from z_mon.filter_prefs import filter_process_matching_func

from z_mon import log_dir
from z_mon.utils import normalize_app_id, parse_size_to_bytes

mibdevider=pow(2,20)
screen=Wnck.Screen.get_default()
has_display = bool(os.environ.get("DISPLAY") or os.environ.get("WAYLAND_DISPLAY"))
icon_theme=g.IconTheme.get_default() if has_display else None
if icon_theme:
...
def _process_cpu_percent(proc, cpu_count):
    try:
        return proc.cpu_percent() / max(cpu_count or 1, 1)
...
def _memory_fields(mem_info):
    rss_value = mem_info.rss if hasattr(mem_info, "rss") else mem_info[0]
    shared_value = getattr(mem_info, "shared", mem_info[2] if len(mem_info) > 2 else 0)
    rss = '{:.1f} MiB'.format(rss_value / mibdevider)
    shared = '{:.1f} MiB'.format(shared_value / mibdevider)
    return rss, shared
...
def sorting_func(model, row1, row2, sort_id):
    ...
    if not isinstance(val1,int):
        val1 = parse_size_to_bytes(val1)
        val2 = parse_size_to_bytes(val2)
    if val1<val2:
        return 1
    elif val1==val2:
        return 0
...
def kill_process(self,widget):
    ...
    if self.selected_process_pid:
        ...
        if response==g.ResponseType.OK:
            try:
                ...
                kill_mode = "1" if previous_killed_process_id==self.selected_process_pid else "0"
                subprocess.run(
                    [
                        "pkexec",
                        os.path.join(os.path.dirname(os.path.abspath(__file__)), "proc-kill.sh"),
                        str(self.selected_process_pid),
                        kill_mode,
                    ],
                    check=True,
                )
            except ProcessLookupError:
                print(f"Process {self.selected_process_pid} already exited.")
            except OSError as exc:
                print(f"Unable to kill process {self.selected_process_pid}: {exc}")
    ...
    # Legacy inline confirmation dialog kept here only as implementation history.
...
class confirmation_popup(g.Dialog):
    ...
    def __init__(self,parentWindow,parent):
       ...
       self.add_buttons(g.STOCK_CANCEL, g.ResponseType.CANCEL, g.STOCK_OK, g.ResponseType.OK)
       self.set_border_width(20)
       content_area=self.get_content_area()
       content_area.add(g.Label('Are you sure you want to end this process?\nPid:{0} Name:{1}'.format(parent.selected_process_pid,parent.processList[parent.selected_process_pid].name())))
       self.show_all()


def _selected_process_from_event(self, treeview, event):
    result = treeview.get_path_at_pos(int(event.x), int(event.y))
    if not result:
        return None
    path, _column, _cell_x, _cell_y = result
    treeview.set_cursor(path)
    model = treeview.get_model()
    iterator = model.get_iter(path)
    pid = model.get_value(iterator, 0)
    self.selected_process_pid = pid
    return pid


def process_tree_button_press(self, treeview, event):
    if event.button != 3:
        return False
    if _selected_process_from_event(self, treeview, event) is None:
        return False
    self.process_context_menu.popup(None, None, None, None, event.button, g.get_current_event_time())
    return True


def set_process_efficiency(self, _widget=None):
    if not self.selected_process_pid:
        return
    try:
        ps.Process(self.selected_process_pid).nice(10)
        ionice = shutil.which("ionice")
        if ionice:
            subprocess.run([ionice, "-c", "3", "-p", str(self.selected_process_pid)], check=False)
    except (ps.AccessDenied, ps.NoSuchProcess, OSError) as exc:
        print(f"Unable to set efficiency mode for {self.selected_process_pid}: {exc}")
    procUpdate(self)


def open_process_location(self, _widget=None):
    if not self.selected_process_pid:
        return
    try:
        folder = os.path.dirname(ps.Process(self.selected_process_pid).exe())
    except (ps.AccessDenied, ps.NoSuchProcess, OSError) as exc:
        print(f"Unable to resolve process location for {self.selected_process_pid}: {exc}")
        return
    opener = shutil.which("xdg-open")
    if opener:
        subprocess.Popen([opener, folder], start_new_session=True)
    else:
        print(f"Process location: {folder}")

def icon_finder(process):
    # global gio_apps
    pname=process.name()
    pid=process.pid
    default_icon=icon_theme.load_icon('application-x-executable', 16, 0) if icon_theme else None
    ...
    for app in gio_apps:
        ...
        if re.search(pname,app_name,re.IGNORECASE) and gicon:
            # print('1st',pname)
            temp=icon_theme.lookup_by_gicon(gicon,16,g.IconLookupFlags.FORCE_SIZE) if icon_theme else None
            if temp:
                return temp.load_icon()

        app_name=normalize_app_id(app.get_id())
        if re.search(pname,app_name,re.IGNORECASE) and gicon:
            # print('2st',pname)
            temp=icon_theme.lookup_by_gicon(gicon,16,g.IconLookupFlags.FORCE_SIZE) if icon_theme else None
            if temp:
...
def searcher(self,sysproc):
    ...
    for procs in childlist:
        if procs.pid not in self.black_list:
            ...
            try:
                cpu_percent=_process_cpu_percent(procs, ps.cpu_count())
                mem_info=procs.memory_info()
            except (ps.NoSuchProcess, ps.ZombieProcess, ps.AccessDenied):
                continue

            cpu_percent="{:.1f} %".format(cpu_percent)
            rss, shared = _memory_fields(mem_info)
            mem_util = rss
            try:
            ...
            for cprocs in procs.children():
                if cprocs.pid not in self.black_list:
                    try:
                        cpu_percent=_process_cpu_percent(cprocs, ps.cpu_count())
                ...
                if self.processSystemd in parents_processess:
                    ...
                    for parent in parents_processess:
                        ...
                        if parent.pid in self.processList:
                            # print(parent.pid)
                            try:
                                cpu_percent=_process_cpu_percent(proc, cpu_count)
        ...
        if widget.get_active():
            if wname=="record_start":
                if self.selected_process_pid:
                    ...
                    self.log_file_writer.writerow(['CPU','Memory','DiskRead','DiskWrite'])
...
def procInit(self):
    ...
    self.processTree.connect("button-press-event", self.process_tree_button_press)
    ...
    self.process_context_menu = g.Menu()
    for label, callback in (
        ("End task", self.kill_process),
        ("Efficiency mode", self.set_process_efficiency),
        ("Open file location", self.open_process_location),
    ):
        item = g.MenuItem(label=label)
        item.connect("activate", callback)
        self.process_context_menu.append(item)
    self.process_context_menu.show_all()
    ...
    self.column_header_list=['pid','Name','Tree CPU','CPU','Tree Memory','Memory','Tree DiskRead','DiskRead','Tree DiskWrite','DiskWrite','Resident\nMemory','Shared','Owner','Command']
    ...
    self.column_select_popover_check_buttons[2].set_active(False)
    self.column_select_popover_check_buttons[4].set_active(False)
...
def procUpdate(self,header=True):
    ...
    self.cpuUtil = ps.cpu_percent()
    self.memPercent = ps.virtual_memory().percent
    ...
    for pidds in reversed_process(tempdi):
        ...
        try:
            if pidds not in pids:
                ...
                # ut=time()
                try:
                    cpu_percent=_process_cpu_percent(self.processList[pidds], cpu_count)
    ...
    if self.record_start and not self.record_pause:
        try:
            if self.log_pid in self.processList:
                ...
                self.log_file_writer.writerow([row[3], row[5], row[7], row[9]])
            else:
                self.process_record_start.set_active(False)
        except Exception as e:
