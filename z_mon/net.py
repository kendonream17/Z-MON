...
try:
    ...
    from z_mon.gproc import byte_to_human
except ImportError:
    from z_mon import files_dir
...
def netinit(self):
    ...
    if len(self.netNameList)!=0:
        ...
        try:
            ...
            netinfo=json.loads(result.stdout) if result.returncode == 0 and result.stdout else []
        except Exception:
            netinfo=[]
...
def netUpdate(self):
    ...
    for i in range(0,self.numOfNets):
        try:
            ...
            self.netWidgetList[i].givedata(self,i)
            self.netWidgetList[i].netdrawarea.queue_draw()
