...
try:
    ...
    from z_mon.gi_composites import GtkTemplate

try:
    from z_mon import files_dir
    from z_mon.gproc import sorting_func, byte_to_human
except ImportError:
    from z_mon import files_dir
...
def diskinit(self):
    ...
    # Getting the name and size of the disks, excluding zrams.
    try:
        ...
        partitions = result.stdout.splitlines() if result.returncode == 0 else []
        for parts in partitions:
            tempparts=parts.split()
            if len(tempparts) >= 3 and tempparts[1] == "disk" and 'zram' not in tempparts[0]:
                self.disklist.append(tempparts[0])
                self.disksize.append(tempparts[2])
    except Exception:
        pass
...
def diskTabUpdate(self):
    ...
    for i in range(0,self.numOfDisks):
        try:
            ...
            self.diskWidgetList[i].givedata(self,i)
            self.diskWidgetList[i].diskdrawarea1.queue_draw()
            self.diskWidgetList[i].diskdrawarea2.queue_draw()
        except Exception:
            continue
